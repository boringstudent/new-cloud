    ...
    <body>
        ...
        <div id="renameModal" class="modal-overlay">
            <div class="modal-content">
                ...
                <h2>重命名文件夹</h2>
                <p style="color: #666; margin-bottom: 15px;">修改文件夹 <strong id="renameOldName"></strong> 的名称</p>
                <div class="form-group">
                    <label>新名称</label>
                    <input type="text" id="renameNewName" placeholder="请输入新名称">
                </div>
                <div class="form-group">
                    <label>用户名</label>
        ...
        <script>
            ...
            function openDeleteFolderModal(filePath, fileName) {{
                isFolderDelete = true;
                deleteFilePath = filePath;
                deleteFileSha = '';
                document.getElementById('deleteFileName').textContent = fileName + '（文件夹，将递归删除所有内容）';
                document.getElementById('deleteModal').classList.add('show');
            }}
            ...
            function handleMenuAction(action) {{
                ...
                if (fileType === 'dir') {{
                    if (action === 'preview') {{
                        ...
                        window.location.href = href + '/' + encodeURIComponent(fileName) + '/';
                    }} else if (action === 'edit') {{
                        ...
                        openDeleteFolderModal(filePath, fileName);
                    }}
                }} else {{
                    if (action === 'preview') {{
                ...
                xhr.onload = function() {{
                    if (xhr.status === 200) {{
                        try {{
                            ...
                            if (response.success && response.key) {{
                                if (isFolderDelete) {{
                                    deleteFolderRecursive(response.key);
                                }} else {{
            ...
            function getSubFiles(folderPath, callback) {{
                var branchXhr = new XMLHttpRequest();
                branchXhr.open('GET', 'https://api.github.com/repos/' + REPO_OWNER + '/' + REPO_NAME + '/branches/' + DEFAULT_BRANCH, true);
                branchXhr.onload = function() {{
                    if (branchXhr.status === 200) {{
                        try {{
                            ...
                            treeXhr.onerror = function() {{ callback(null); }};
                            treeXhr.send();
                        }} catch (e) {{
                            callback(null);
                        }}
                ...
                branchXhr.onerror = function() {{ callback(null); }};
                branchXhr.send();
            }}

            function openRenameModal(filePath, oldName) {{
            ...
            function confirmRename() {{
                ...
                if (!newName || newName === renameOldName) {{
                    showRenameMessage('请输入有效的新名称', 'error');
                    return;
                }}
                ...
                xhr.onload = function() {{
                    if (xhr.status === 200) {{
                        try {{
                            var response = JSON.parse(xhr.responseText);
                            if (response.success && response.key) {{
                                renameFolderFiles(response.key, newName);
                            }} else {{
            ...
            function renameFolderFiles(key, newName) {{
                showRenameMessage('正在获取文件夹内容...', 'success');

                getSubFiles(renameFilePath, function(files) {{
                    if (files === null) {{
                        showRenameMessage('获取文件夹内容失败', 'error');
                        document.getElementById('renameBtn').disabled = false;
                        return;
                    }}

                    if (files.length === 0) {{
                        showRenameMessage('文件夹为空', 'error');
                        document.getElementById('renameBtn').disabled = false;
                        return;
                    }}

                    var oldPathParts = renameFilePath.split('/');
                    var newPrefix = oldPathParts.slice(0, -1).join('/');
                    if (newPrefix) newPrefix += '/';
                    newPrefix += newName;

                    showRenameMessage('正在重命名 ' + files.length + ' 个文件...', 'success');
                    ...
                    function processNext(index) {{
                        if (index >= files.length) {{
                            if (failedCount === 0) {{
                                showRenameMessage('重命名成功！（共 ' + doneCount + ' 个文件）', 'success');
                                setTimeout(function() {{
                                    closeRenameModal();
                                    loadFileList();
                                }}, 1500);
                            }} else {{
                                showRenameMessage('完成，' + doneCount + ' 成功，' + failedCount + ' 失败', 'error');
                                document.getElementById('renameBtn').disabled = false;
                            }}
                            return;
                        }}
                        ...
                        var oldFullPath = file.path;
                        var relativePath = oldFullPath.substring(renameFilePath.length);
                        var newFullPath = newPrefix + relativePath;

                        var getXhr = new XMLHttpRequest();
                        getXhr.open('GET', 'https://api.github.com/repos/' + REPO_OWNER + '/' + REPO_NAME + '/contents/' + encodeURI(oldFullPath), true);
                        getXhr.setRequestHeader('Authorization', 'Bearer ' + key);
                        getXhr.onload = function() {{
                            if (getXhr.status === 200) {{
                                try {{
                                    var fileData = JSON.parse(getXhr.responseText);
                                    var createXhr = new XMLHttpRequest();
                                    createXhr.open('PUT', 'https://api.github.com/repos/' + REPO_OWNER + '/' + REPO_NAME + '/contents/' + encodeURI(newFullPath), true);
                                    ...
                                    createXhr.onload = function() {{
                                        if (createXhr.status === 200 || createXhr.status === 201) {{
                                            var delXhr = new XMLHttpRequest();
                                            delXhr.open('DELETE', 'https://api.github.com/repos/' + REPO_OWNER + '/' + REPO_NAME + '/contents/' + encodeURI(oldFullPath), true);
                                            ...
                                            delXhr.onload = function() {{
                                                if (delXhr.status === 200) doneCount++;
                                                else failedCount++;
                                                showRenameMessage('重命名中... ' + (index + 1) + '/' + files.length, 'success');
                                                processNext(index + 1);
                                            }};
                                            delXhr.onerror = function() {{ failedCount++; processNext(index + 1); }};
                                            delXhr.send(JSON.stringify({{message: 'Rename: delete old ' + oldFullPath, sha: fileData.sha}}));
                                        }} else {{
                                            failedCount++;
                                            processNext(index + 1);
                                        }}
                                    }};
                                    createXhr.onerror = function() {{ failedCount++; processNext(index + 1); }};
                                    createXhr.send(JSON.stringify({{message: 'Rename: ' + renameOldName + ' -> ' + newName, content: fileData.content}}));
                                }} catch (e) {{
                                    failedCount++;
                                    processNext(index + 1);
                                }}
                        ...
                        getXhr.onerror = function() {{ failedCount++; processNext(index + 1); }};
                        getXhr.send();
                    }}
            ...
            function deleteFolderRecursive(key) {{
                showDeleteMessage('正在获取文件夹内容...', 'success');

                getSubFiles(deleteFilePath, function(files) {{
                    if (files === null) {{
                    ...
                    if (files.length === 0) {{
                        showDeleteMessage('文件夹为空', 'error');
                        document.getElementById('deleteBtn').disabled = false;
                        isFolderDelete = false;
                        return;
                    }}

                    showDeleteMessage('正在删除 ' + files.length + ' 个文件...', 'success');
                    deleteFolderStep(key, files, 0, 0, 0);
                }});
            }}

            function deleteFolderStep(key, files, index, doneCount, failedCount) {{
                if (index >= files.length) {{
                    if (failedCount === 0) {{
                        showDeleteMessage('删除成功！（共 ' + doneCount + ' 个文件）', 'success');
                    }} else {{
                        showDeleteMessage('完成，' + doneCount + ' 成功，' + failedCount + ' 失败', 'error');
                    }}
                    isFolderDelete = false;
                    setTimeout(function() {{
                        closeDeleteModal();
                ...
                var file = files[index];
                var getXhr = new XMLHttpRequest();
                getXhr.open('GET', 'https://api.github.com/repos/' + REPO_OWNER + '/' + REPO_NAME + '/contents/' + encodeURI(file.path), true);
                getXhr.setRequestHeader('Authorization', 'Bearer ' + key);
                getXhr.onload = function() {{
                    if (getXhr.status === 200) {{
                        try {{
                            var fileData = JSON.parse(getXhr.responseText);
                            var delXhr = new XMLHttpRequest();
                            delXhr.open('DELETE', 'https://api.github.com/repos/' + REPO_OWNER + '/' + REPO_NAME + '/contents/' + encodeURI(file.path), true);
                            delXhr.setRequestHeader('Authorization', 'Bearer ' + key);
                            delXhr.setRequestHeader('Content-Type', 'application/json');
                            delXhr.onload = function() {{
                                var d = delXhr.status === 200 ? doneCount + 1 : doneCount;
                                var f = delXhr.status === 200 ? failedCount : failedCount + 1;
                                showDeleteMessage('删除中... ' + (index + 1) + '/' + files.length, 'success');
                                deleteFolderStep(key, files, index + 1, d, f);
                            }};
                            delXhr.onerror = function() {{
                                showDeleteMessage('删除中... ' + (index + 1) + '/' + files.length, 'success');
                                deleteFolderStep(key, files, index + 1, doneCount, failedCount + 1);
                            }};
                            delXhr.send(JSON.stringify({{message: 'Delete: ' + file.path, sha: fileData.sha}}));
                        }} catch (e) {{
                            deleteFolderStep(key, files, index + 1, doneCount, failedCount + 1);
                        }}
                    }} else {{
            ...
            function downloadFolderAsZip(folderPath, folderName) {{
                showMessage('正在获取文件夹内容...', 'success');
                document.getElementById('uploadBtn').disabled = true;

                getSubFiles(folderPath, function(files) {{
                    if (files === null || files.length === 0) {{
                        showMessage('文件夹为空或获取失败', 'error');
                        document.getElementById('uploadBtn').disabled = false;
                        return;
                    }}

                    showMessage('正在打包 ' + files.length + ' 个文件...', 'success');
                    document.querySelector('.progress-container').style.display = 'block';

                    var zip = new JSZip();
                    var doneCount = 0;
                    var failedCount = 0;
                    var prefixRemove = folderPath + '/';

                    function processNext(index) {{
                        if (index >= files.length) {{
                            var percent = Math.round((doneCount / files.length) * 100);
                            document.getElementById('progressFill').style.width = percent + '%';
                            document.getElementById('progressText').textContent = '生成压缩包...';

                            zip.generateAsync({{type: 'blob'}}, function(meta) {{
                                document.getElementById('progressText').textContent = Math.round(meta.percent) + '%';
                            }}).then(function(blob) {{
                                ...
                                showMessage('下载完成！（共 ' + (files.length - failedCount) + ' 个文件' + (failedCount > 0 ? '，' + failedCount + ' 个失败' : '') + '）', 'success');
                                document.getElementById('uploadBtn').disabled = false;
                            }});
                            return;
                        }}
                        ...
                        var file = files[index];
                        var relativePath = file.path.substring(prefixRemove.length);

                        var percent = Math.round((index / files.length) * 100);
                        document.getElementById('progressFill').style.width = percent + '%';
                        document.getElementById('progressText').textContent = (index + 1) + '/' + files.length + ' ' + relativePath;

                        var xhr = new XMLHttpRequest();
                        xhr.responseType = 'arraybuffer';
                        xhr.open('GET', 'https://raw.githubusercontent.com/' + REPO_OWNER + '/' + REPO_NAME + '/' + DEFAULT_BRANCH + '/' + encodeURI(file.path).replace(/%2F/g, '/'), true);
                        xhr.onload = function() {{
                            if (xhr.status === 200) {{
                                zip.file(relativePath, xhr.response);
                                doneCount++;
                            }} else {{
                                failedCount++;
                            }}
            ...
            function uploadFile() {{
                ...
                if (fileInput.files && fileInput.files.length > 0) {{
                    for (var i = 0; i < fileInput.files.length; i++) {{
                        allFiles.push({{file: fileInput.files[i], relativePath: fileInput.files[i].name}});
                    }}
                }}
                ...
                if (allFiles.length === 0) {{
                    showMessage('请选择要上传的文件或文件夹', 'error');
                    return;
                }}
                ...
                xhr.onload = function() {{
                    if (xhr.status === 200) {{
                        try {{
                            var response = JSON.parse(xhr.responseText);
                            if (response.success && response.key) {{
                                uploadMultipleToGitHub(response.key, allFiles);
                            }} else {{
            ...
            function uploadMultipleToGitHub(key, fileList) {{
                ...
                function uploadNext(index) {{
                    if (index >= fileList.length) {{
                        ...
                        document.getElementById('progressText').textContent = totalFiles + '/' + totalFiles;
                        if (failedFiles.length === 0) {{
                            showMessage('全部上传成功！（共 ' + totalFiles + ' 个文件）', 'success');
                        }} else {{
                            showMessage('上传完成，' + uploadedCount + ' 个成功，' + failedFiles.length + ' 个失败', 'error');
                        }}
                        document.getElementById('uploadBtn').disabled = false;
                        setTimeout(function() {{
                            closeUploadModal();
                    ...
                    var percent = Math.round((index / totalFiles) * 100);
                    document.getElementById('progressFill').style.width = percent + '%';
                    document.getElementById('progressText').textContent = (index + 1) + '/' + totalFiles + ' ' + file.name;

                    var reader = new FileReader();
                    reader.onload = function(e) {{
                        var base64Content = e.target.result.split(',')[1];
                        var currentPath = getCurrentPath();
                        var filePath = currentPath ? currentPath + '/' + relativePath : relativePath;
                        ...
                        uploadXhr.onload = function() {{
                            if (uploadXhr.status === 200 || uploadXhr.status === 201) {{
                                uploadedCount++;
                                showMessage('已上传: ' + relativePath + ' (' + (index + 1) + '/' + totalFiles + ')', 'success');
                            }} else {{
                                var errMsg = '状态码: ' + uploadXhr.status;
                                try {{
                                    var errObj = JSON.parse(uploadXhr.responseText);
                                    errMsg = errObj.message || errMsg;
                                }} catch (ex) {{}}
                                failedFiles.push(relativePath + ' - ' + errMsg);
                            }}
                            uploadNext(index + 1);
                        }};

                        uploadXhr.onerror = function() {{
                            failedFiles.push(relativePath + ' - 网络错误');
                            uploadNext(index + 1);
                        }};
